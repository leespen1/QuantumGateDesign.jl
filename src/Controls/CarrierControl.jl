...
"""
Calculate the values of p⁽ⁱ⁾(t)/i!, and the same for q, for p+iq = eⁱʷᵗ, for
each carrier frequency w. Store them in a matrix, each column has the value and derivatives.
Columns are ordered by p/q, then by carrier frequency.
"""
    ...
    if (t != control.current_t[]) || (nderiv > control.current_derivative_order[])
        ...
        for (i, w) in enumerate(control.carrier_frequencies)
            ...
            for k in 0:nderiv
                ...
                control.carrier_val_storage[1+k, 1, i] = carrier_pval / factorial(k)
                control.carrier_val_storage[1+k, 2, i] = carrier_qval / factorial(k)
                w_pow *= w
            end
        end
        updated = true
...
"""
Calculate the gradients of the derivatives of the base control, store them in a
matrix.
...
@inline function update_base_vals!(control::CarrierControl, t::Real, nderiv::Integer, pcof::AbstractVector{<: Real})
    for i in 1:control.N_frequencies
        ...
        base_p_vec = view(control.base_val_storage, 1:1+nderiv, 1, i)
        base_q_vec = view(control.base_val_storage, 1:1+nderiv, 2, i)
